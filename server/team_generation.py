...
class TeamBuilder:
    def __init__(self, team_owner):
        ...
        __names_file = open('/home/davinci/Desktop/hacks/stumped/server/Indian-Male-Names.csv', 'r')
        ...
        self.__bowling_types = ('Right arm fast', 'Right arm medium', 'Left arm fast', 'Left arm medium', 'Off break', 'Leg break', 'Slow left arm orthodox')
        self.__fitness_levels = ('Poor', 'Decent', 'Good', 'Superb')
        self.__skill_levels = ('Non-existent', 'Horrible', 'Hopeless', 'Useless', 'Mediocre', 'Average', 'Reliable', 'Accomplished', 'Remarkable', 'Brilliant', 'Exemplary', 'Prodigious', 'Fantastic', 'Magnificient', 'Masterful', 'Legendary', 'Supreme', 'Magical', 'Demigod', 'Godlike')
    ...
    def build_new_team(self, team_name):
        ...
        self.__players = [{'player_type': 'batter'}] * 5 + [{'player_type': 'wicket-keeper'}] * 2 + [{'player_type': 'allrounder'}] * 3 + [{'player_type': 'bowler'}] * 5
    ...
    def get_new_player(self, player):
        ...
        player['batting_rating'] = math.ceil((self.__skill_levels.index(player['batting_seam']) +  self.__skill_levels.index(player['batting_spin']))/2 + 1)
        player['bowling_rating'] = math.ceil((self.__skill_levels.index(player['bowling_main']) +  self.__skill_levels.index(player['bowling_variation']))/2 + 1)
        return player


def get_batting_bowling_lineup(team):
    playing_eleven = random.sample(team['players'][0:5], 4) + \
...
team1 = team.build_new_team('GG1')
team1['team_name'] = team1['team_id'] = 'GG1'

team2 = team.build_new_team('GG2')
team2['team_name'] = team2['team_id'] = 'GG2'

team1['batting_lineup'], team1['bowling_lineup'] = get_batting_bowling_lineup(team1)
team2['batting_lineup'], team2['bowling_lineup'] = get_batting_bowling_lineup(team2)
